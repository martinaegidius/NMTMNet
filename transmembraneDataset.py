...
class transmembraneDataset(gDataset):
    ...
    def __init__(self, root, proteinlist, mismatching_proteins,labelDict, flush_files = False, transform=None, pre_transform=None,pre_filter=None):
    ...
    def processed_file_names(self):
        return ["protein_ "+ str(i) + ".pt" for i,_ in enumerate(self.proteinlist)]
    ...
    def process(self):
        ...
        all_files_tmp = glob.glob(osp.join(self.processed_dir, 'protein_*.pt'))
        ...
        for i, raw_path in enumerate(self.raw_paths):
            print(f"running protein {i}/{len(self.raw_paths)}")
            tmp_file_check = osp.join(self.processed_dir, f'protein_{idx}.pt')
            if(os.path.isfile(tmp_file_check) and self.flush_files==False): #case: some files may already exist due to interruption of earlier command
                ...
                self.conversionDict[id] = f'protein_{idx}.pt'
                #print("the path join has arg:")
                #print(osp.join(self.processed_dir, f'protein_{idx}.pt'))
                torch.save(protein, osp.join(self.processed_dir, f'protein_{idx}.pt'))
            idx += 1
    ...
    def get(self,idx):
        data = torch.load(osp.join(self.processed_dir, f'protein_{idx}.pt'),map_location='cuda')
