...
def saveResults(session_id,root_path,trainloss,train_pr_acc,train_O_acc,valloss,val_pr_acc,val_O_acc,test,test_predictions,train_acc,val_acc,test_acc,train_acc_overlap,val_acc_overlap,test_acc_overlap,param_cfg):
  ...
  with open(pwd_tmp+"/train.txt", "w") as f:
      f.write(json.dumps(trainloss))
  
  with open(pwd_tmp+"/val.txt", "w") as f:
      f.write(json.dumps(valloss))
  ...
  with open(pwd_tmp+"/train_epoch_pr_accuracies.pkl","w") as f:
    f.write(json.dumps(train_pr_acc))
  
  with open(pwd_tmp+"/train_epoch_overlap_accuracies.pkl","w") as f:
    f.write(json.dumps(train_O_acc))
  
  with open(pwd_tmp+"/val_epoch_pr_accuracies.pkl","w") as f:
    f.write(json.dumps(val_pr_acc))

  with open(pwd_tmp+"/val_epoch_overlap_accuracies.pkl","w") as f:
    f.write(json.dumps(val_O_acc))
  
  return None

def saveProteinMetrics(session_id,root_path,residue_confusion,type_confusion,metricDict,SetType):
  pwd_tmp = os.path.join(root_path,"ExperimentalResults")
  #print(pwd_tmp)
  if not (os.path.exists(pwd_tmp)):
    os.mkdir(pwd_tmp)
  ...
  if not (os.path.exists(pwd_tmp)):
    os.mkdir(pwd_tmp) #make dir for session id - this is unique, so needs to made every time
  pwd_tmp = pwd_tmp+"/"+SetType+"_"
  torch.save(residue_confusion,pwd_tmp+"residue_confusion.pt")
  torch.save(type_confusion,pwd_tmp+"type_confusion.pt")
  with open(pwd_tmp+"proteinMetrics.json","w") as f:
...
def dataset_accuracy(model,dataloader,type2key,mode="null",metrics=False): #trying with colab version
    ...
    with torch.no_grad():
      ...
      for i, data in enumerate(dataloader):
          ...
          for k, output_ in enumerate(output): #go through all outputs
            ...
            incorrect_single += torch.sum((preds_!=label_))
          
            if(k==0):
               probs = probs_
               preds = preds_
            else:
               probs = torch.cat((probs,probs_),0)
               preds = torch.cat((preds,preds_),0) #basically rebatching
          
            preds_top = label_list_to_topology(preds_)
            label_top = label_list_to_topology(label_)
    ...
    if(metrics):
       ...
       result_metrics = topology_accuracies(top_confusion_matrix) #dict of results, save to scratch
...
def log_batch_elementwise_accuracies(preds_li,label_li,id_li):
  ...
  #   num_samples = 1
  num_samples = len(preds_li)
  for j in range(num_samples):
    #print(f"Investigating samle no ", j)
    prot = preds_li[j] #numeric prediction tensor
    label = label_li[j]
    ...
    ids.append(id_li[j].replace("_ABCD",""))
...
def train_single_epoch(model,optimizer,criterion,trainloader,type2key,gradclip=False,clipval=None,DEBUG=False):
    ...
    for i, data in enumerate(trainloader):
        #print(data)
        output, protein_lengths = model(data,batch=data.batch) #output is a list of per-protein probabilities and a list indicating the length of the individuals proteins in the batch
        batch_sz = len(output)
